...
"""
struct StochasticBlockNetworkHawkesProcess <: HawkesProcess
    # parameters
    λ0::Array{Float64,1}
    μ::Array{Float64,2}
    τ::Array{Float64,2}
    ...
    γ::Float64
    K::Int32
    function StochasticBlockNetworkHawkesProcess(λ0, μ, τ, A, W, Δtmax, N, α0, β0, κ, ν, μμ, κμ, ατ, βτ, αρ, βρ, γ, K)
        ...
        return new(λ0, μ, τ, A, W, Δtmax, N, α0, β0, κ, ν, μ_μ, κ_μ, α_τ, β_τ, α_ρ, β_ρ, γ, K)
    end
end

function rand(p::StochasticBlockNetworkHawkesProcess, T::Float64)
...
function loglikelihood(p::StochasticBlockNetworkHawkesProcess, events, nodes, parents)
    nchannels = length(p.λ0)
    ll = 0.
...
function intensity(p::StochasticBlockNetworkHawkesProcess, events, nodes, t0)
    nchannels = length(p.λ0)
...
function impulse_response(p::StochasticBlockNetworkHawkesProcess, parentindex, childindex)
    P = LogitNormalProcess.(p.W, p.μ, p.τ, Δtmax)
    return intensity.(P)
end

stability(p::StochasticBlockNetworkHawkesProcess) = maximum(abs(p.A .* p.W))
