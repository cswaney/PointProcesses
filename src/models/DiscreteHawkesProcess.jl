mutable struct DiscreteHawkesProcess
    ...
    κ0::Float64
    ν0::Float64
    κ1::Float64  # used w/ VI
    ν1::Float64  # used w/ VI
    # network
    net::Network
end
...
Calculate discretized Gaussian basis functions with means are evenly spaced along `[1, L]`.
"""
function basis(p::DiscreteHawkesProcess)
    ...
    σ = L / (B - 1)
    if L <= 2
        μ  = Array(LinRange(1, L, B + 2)[2:end - 1])
    else
        μ  = Array(LinRange(1, L, B))
    end
    lags = Array(1:L)
    ...
    for t = 1:T - 1
        ...
        for n = 1:N
            ...
            for s = 1:S[n, t]
                ...
                # @info "Lmax=$Lmax"
                for d in 1:Lmax
                    λ[:, d] = p.A[n, :] .* p.W[n, :] .* IR[n, :, d]  # 1 x N x L
                end
                children = rand.(Poisson.(λ .* Δt))
                S[:, t + 1:t + Lmax] .+= children
...
#     α, β = update_link_probabilities(p.net, ρ)
#     return α, β
# end
#
#
